...
import { PROZA_1KLASS_STORIES, type ProzaStory } from '@/lib/proza-1klass';
import { praiseFor } from '@/lib/praise';

const ROUNDS_PER_SESSION = Math.min(10, PROZA_1KLASS_STORIES.length);
...
function buildSession(): ProzaStory[] {
  return shuffle(PROZA_1KLASS_STORIES).slice(0, ROUNDS_PER_SESSION);
}

export default function Proza1KlassTrainerPage() {
  const [started, setStarted] = useState(false);
  const [session, setSession] = useState<ProzaStory[]>([]);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">📖 Проза — читай и понимай</h1>
      </div>

      <TrainerGate type="trainer:proza-1klass">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card text-center">
              <p className="text-lg text-white/90 mb-6">
                Прочитай короткий рассказ и ответь на вопрос. Всего {ROUNDS_PER_SESSION} рассказов.
              </p>
              ...
              {showQuestion && (
                <>
                  ...
                  <div className="h-8">
                    ...
                    {feedback === 'wrong' && (
                      <p className="text-red-500 font-black text-xl">Перечитай рассказ ещё раз 👀</p>
                    )}
                  </div>
                </>
              )}
