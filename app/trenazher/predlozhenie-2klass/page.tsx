...
import { PREDLOZHENIE_SENTENCES, PredlozhenieType } from '@/lib/predlozhenie-2klass';
import { praiseFor } from '@/lib/praise';

const TYPE_LABELS: Record<PredlozhenieType, string> = {
  povestv: 'Повествовательное',
  vopros: 'Вопросительное',
  pobud: 'Побудительное',
};

const TYPE_HINTS: Record<PredlozhenieType, string> = {
  povestv: 'сообщает, рассказывает',
  vopros: 'спрашивает',
  pobud: 'просит, приказывает',
};

const TYPES: PredlozhenieType[] = ['povestv', 'vopros', 'pobud'];
...
function makeSession() {
  return shuffle(PREDLOZHENIE_SENTENCES).slice(0, ROUNDS_PER_SESSION);
}

export default function Predlozhenie2klassTrainerPage() {
  const [started, setStarted] = useState(false);
  const [session, setSession] = useState(() => PREDLOZHENIE_SENTENCES.slice(0, ROUNDS_PER_SESSION));
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">💬 Виды предложений по цели высказывания</h1>
      </div>

      <TrainerGate type="trainer:predlozhenie-2klass">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card bg-white text-center py-10">
              <p className="text-3xl font-black text-[#3a1c6e] mb-4">💬 Виды предложений</p>
              <p className="text-gray-600 mb-8 text-lg">
                Прочитай предложение и выбери, какое оно: повествовательное, вопросительное или побудительное.
              </p>
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-lg text-gray-500 mb-4">Какое это предложение по цели высказывания?</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {TYPES.map((type) => {
                  ...
                  return (
                    ...
                    >
                      <span className="block mb-1">{TYPE_LABELS[type]}</span>
                      <span className="block text-xs font-normal text-gray-500">{TYPE_HINTS[type]}</span>
                    </button>
                  );
                })}
              </div>
