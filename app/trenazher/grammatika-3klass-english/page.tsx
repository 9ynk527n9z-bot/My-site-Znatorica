...
import { GRAMMAR_TASKS, GrammarTask } from '@/lib/grammatika-3klass-english';
...
function makeSession(): GrammarTask[] {
  return shuffle(GRAMMAR_TASKS).slice(0, ROUNDS_PER_SESSION);
}

export default function Grammatika3klassEnglishTrainerPage() {
  const [started, setStarted] = useState(false);
  const [session, setSession] = useState<GrammarTask[]>([]);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🇬🇧 Грамматика (английский, 3 класс)</h1>
      </div>

      <TrainerGate type="trainer:grammatika-3klass-english">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card bg-white text-center py-10">
              <p className="text-3xl font-black text-[#3a1c6e] mb-4">🇬🇧 Грамматика</p>
              <p className="text-gray-600 mb-8 text-lg">
                Вставь пропущенное слово: to be (am/is/are), окончания -s/-es в Present Simple
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-lg text-gray-500 mb-4">Выбери правильное слово:</p>
              ...
              >
                {parts[0]}
                <span className="inline-block px-3 border-b-4 border-orange text-orange">
                  {feedback && pickedIndex !== null ? current.options[pickedIndex] : '___'}
                </span>
                {parts[1]}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {current.options.map((option, index) => {
                  ...
                  const isCorrectAnswer = !!feedback && index === current.answer;
                  const showWrong = !!feedback && isPicked && index !== current.answer;
                  return (
                    <button
                      key={option + index}
              ...
              <div className="min-h-[3rem]">
                ...
                {feedback === 'wrong' && (
                  <div>
                    <p className="text-red-500 font-black text-xl mb-1">
                      Правильно: {current.options[current.answer]} 👀
                    </p>
