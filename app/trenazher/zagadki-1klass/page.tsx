...
import { RIDDLES_1KLASS, type Riddle } from '@/lib/riddles-1klass';
...
function makeSession(): Round[] {
  const pool = shuffle(RIDDLES_1KLASS).slice(0, ROUNDS_PER_SESSION);
  return pool.map((riddle) => {
    const distractors = shuffle(RIDDLES_1KLASS.filter((r) => r.id !== riddle.id)).slice(0, 3);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🧩 Загадки для 1 класса</h1>
      </div>

      <TrainerGate type="trainer:zagadki-1klass">
          ...
          {finished && (
            <div className="card bg-white text-center py-10">
              ...
              <div className="flex justify-center gap-3 flex-wrap">
                ...
                <Link href="/1-klass/logika/zagadki" className="btn-secondary px-6 py-3">
                  📖 К теории
                </Link>
              </div>
            </div>
