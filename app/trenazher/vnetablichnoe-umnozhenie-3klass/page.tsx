...
// Умножение: двузначное число (11-33) на однозначное (2-9), результат не больше 99.
function makeMultiplicationRound(): Round {
  for (let attempt = 0; attempt < 200; attempt++) {
    const a = randInt(11, 33);
    const b = randInt(2, 9);
    const answer = a * b;
    if (answer > 99) continue;
    return { expression: `${a} × ${b}`, answer, options: makeOptions(answer) };
  }
  // Фолбэк — гарантированно корректный пример
  return { expression: '23 × 3', answer: 69, options: makeOptions(69) };
}

// Деление: делимое (20-99) на делитель (2-9), делится БЕЗ остатка (считаем реальным кодом).
function makeDivisionRound(): Round {
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    return { expression: `${dividend} ÷ ${divisor}`, answer, options: makeOptions(answer) };
  }
  // Фолбэк — гарантированно корректный пример
  return { expression: '96 ÷ 4', answer: 24, options: makeOptions(24) };
}

// Чередуем умножение и деление строго по чётности раунда.
function makeRound(roundNumber: number): Round {
  return roundNumber % 2 === 1 ? makeMultiplicationRound() : makeDivisionRound();
}
...
function makeOptions(answer: number): number[] {
  ...
  while (options.size < 4 && guard < 100) {
    ...
    if (candidate >= 0 && candidate <= 300 && candidate !== answer) {
...
export default function VnetablichnoeUmnozhenie3klassTrainerPage() {
  ...
  function nextRound() {
    ...
    setCurrent(makeRound(nextRoundNumber));
    setFeedback(null);
    setPickedValue(null);
  }
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🧮 Внетабличное умножение и деление (3 класс)</h1>
      </div>

      <TrainerGate type="trainer:vnetablichnoe-umnozhenie-3klass">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card mb-8 text-center">
              <p className="text-lg text-white/90 mb-4">
                Реши 10 примеров: умножение двузначного числа на однозначное и деление без остатка.
                Раскладывай числа по разрядам — это самый быстрый способ!
              </p>
