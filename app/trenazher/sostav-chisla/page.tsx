...
function makeRound(): Round {
  ...
  const maxWrong = 3;
  let guard = 0;
  while (optionsSet.size < maxWrong + 1 && guard < 50) {
    guard++;
    const delta = randInt(-3, 3);
    const candidate = answer + delta;
    if (candidate >= 0 && candidate <= total && candidate !== answer) {
  ...
  return { total, known, knownIsFirst, answer, options };
}

const ROUNDS_PER_SESSION = 10;

export default function SostavChislaTrainerPage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🏠 Состав числа</h1>
      </div>

      <TrainerGate type="trainer:sostav-chisla">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card text-center py-10">
              <p className="text-xl font-bold text-white mb-6">
                Реши 10 «домиков»: найди недостающее слагаемое числа от 5 до 10
              </p>
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-xl font-bold text-[#3a1c6e] mb-6">Какое слагаемое пропущено в домике?</p>

              {/* Домик */}
              <div className="flex flex-col items-center mb-8">
                <div
                  className="relative flex items-center justify-center w-40 h-40 rounded-2xl text-white text-7xl font-black mb-3"
                ...
                <div className="flex items-center justify-center gap-6">
                  <div
                    className={`flex flex-col items-center justify-center w-40 h-40 rounded-2xl border-4 font-black gap-1 ${
                      current.knownIsFirst
                        ? 'border-orange bg-orange/10 text-[#3a1c6e]'
                        : 'border-dashed border-gray-300 bg-gray-50 text-gray-300'
                    }`}
                  >
                    {current.knownIsFirst ? (
                      <>
                        <span className="text-lg leading-none">{'⭐'.repeat(Math.min(current.known, 12))}</span>
                        <span className="text-6xl">{current.known}</span>
                      </>
                    ) : (
