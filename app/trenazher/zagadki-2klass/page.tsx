...
import { RIDDLES_2KLASS, type Riddle } from '@/lib/riddles-2klass';
...
function makeSession(): Round[] {
  const pool = shuffle(RIDDLES_2KLASS).slice(0, ROUNDS_PER_SESSION);
  return pool.map((riddle) => {
    const distractors = shuffle(RIDDLES_2KLASS.filter((r) => r.id !== riddle.id)).slice(0, 3);
    return { riddle, options: shuffle([riddle, ...distractors]) };
  });
}

export default function Zagadki2klassTrainerPage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🧩 Загадки для 2 класса</h1>
      </div>

      <TrainerGate type="trainer:zagadki-2klass">
          ...
          {!started && (
            <div className="card text-center py-10">
              ...
              <p className="text-gray-600 mb-8">
                Загадки для второклассников устроены хитрее: подсказки скрыты в метафорах.
                Прочитай загадку и выбери правильный ответ из четырёх картинок. 10 раундов, без повторов.
              </p>
          ...
          {finished && (
            <div className="card bg-white text-center py-10">
              ...
              <div className="flex justify-center gap-3 flex-wrap">
                ...
                <Link href="/2-klass/logika/zagadki" className="btn-secondary px-6 py-3">
                  📖 К теории
                </Link>
              </div>
            </div>
