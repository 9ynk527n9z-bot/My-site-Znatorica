...
import { pickPerelivanieTasks4Klass, type PerelivanieTask4Klass } from '@/lib/perelivanie-4klass';
...
export default function Perelivanie4KlassTrainerPage() {
  const [started, setStarted] = useState(false);
  const [rounds, setRounds] = useState<PerelivanieTask4Klass[]>([]);
  ...
  function begin() {
    ...
    setRounds(pickPerelivanieTasks4Klass(ROUNDS_PER_SESSION));
    setFeedback(null);
    setPickedIndex(null);
  }
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🧪 Задачи на переливание (4 класс)</h1>
      </div>

      <TrainerGate type="trainer:perelivanie-4klass">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card mb-8 text-center">
              <p className="text-lg text-white/90 mb-6">
                Реши 10 задач на переливание: два сосуда без делений и кран с водой.
                Для каждой задачи нужно посчитать, сколько минимум ходов (наполнить,
                перелить или вылить воду) потребуется, чтобы отмерить нужный объём.
              </p>
