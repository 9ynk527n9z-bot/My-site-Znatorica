...
function makeRound(): QuizRound {
  const answer = randItem(RUSSIAN_NUMBERS);
  const options = [answer];
  while (options.length < 4) {
    const candidate = randItem(RUSSIAN_NUMBERS);
...
export default function SchetDo10TrainerPage() {
  ...
  const current = RUSSIAN_NUMBERS[index];

  function goPrev() {
    setIndex((i) => (i - 1 + RUSSIAN_NUMBERS.length) % RUSSIAN_NUMBERS.length);
  }

  function goNext() {
    setIndex((i) => (i + 1) % RUSSIAN_NUMBERS.length);
  }
    ...
    <div className="bg-black min-h-screen">
      <div className="bg-[#1E1035] border-b border-[#2D2350] px-6 py-4 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold mt-2">🔢 Счёт до 10</h1>
        </div>
      ...
      <TrainerGate type="trainer:schet-do-10">
        {tab === 'study' && (
          <div className="max-w-2xl mx-auto py-10 px-6">
            ...
            <div className="card bg-white text-center py-10 px-6">
              <p className="text-gray-500 text-sm mb-4">
                Число {index + 1} из {RUSSIAN_NUMBERS.length}
              </p>
            ...
            {finished && (
              <div className="card bg-white text-center py-10">
                ...
                <div className="mt-6 pt-5 border-t border-gray-200">
                  <ShareButtons
                    text={shareTextFor('Счёт до 10', score.correct, score.total)}
                    url="https://znatorica.ru/trenazher/schet-do-10"
                    trackKey="game:schet-do-10"
                  />
                </div>
              </div>
            )}
