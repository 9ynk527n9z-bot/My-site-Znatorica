...
function makeRound(): Round {
  ...
  const type: QuestionType = Math.random() < 0.5 ? 'tens' : 'units';
  const answer = type === 'tens' ? tens : units;

  const wrongPool = new Set<number>();
  const maxValue = type === 'tens' ? 9 : 9;
  const minValue = 0;
...
export default function DvuznachnyeTrainerPage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🔢 Двузначные числа</h1>
      </div>

      <TrainerGate type="trainer:dvuznachnye">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card mb-8 text-center">
              <p className="text-lg text-white/90 mb-6">
                Определи, сколько десятков и сколько единиц в двузначном числе. Выбери правильный ответ из вариантов!
              </p>
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-xl font-bold text-[#3a1c6e] mb-8">
                Сколько {current.type === 'tens' ? 'десятков' : 'единиц'} в числе {current.number}?
              </p>
