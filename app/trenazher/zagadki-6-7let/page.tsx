...
import { RIDDLES_6_7LET, type Riddle } from '@/lib/zagadki-6-7let';
...
function makeSession(): Round[] {
  const pool = shuffle(RIDDLES_6_7LET).slice(0, ROUNDS_PER_SESSION);
  return pool.map((riddle) => {
    const distractors = shuffle(RIDDLES_6_7LET.filter((r) => r.id !== riddle.id)).slice(0, 3);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🤔 Загадки для дошкольников 6–7 лет</h1>
      </div>

      <TrainerGate type="trainer:zagadki-6-7let">
          ...
          {finished && (
            <div className="card bg-white text-center py-10">
              ...
              <div className="flex justify-center gap-3 flex-wrap">
                ...
                <Link href="/6-7-let/logika/zagadki" className="btn-secondary px-6 py-3">
                  📖 К теории
                </Link>
              </div>
            </div>
