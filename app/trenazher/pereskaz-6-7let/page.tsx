...
import { STORIES_6_7, type Story } from '@/lib/pereskaz-6-7let';
import { shuffle } from '@/lib/shapes';
import { praiseFor } from '@/lib/praise';

const ROUNDS_PER_SESSION = STORIES_6_7.length;
...
export default function Pereskaz67LetTrainerPage() {
  ...
  function begin() {
    const order = shuffle(STORIES_6_7);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🖼️ Пересказ по картинкам (6–7 лет)</h1>
      </div>

      <TrainerGate type="trainer:pereskaz-6-7let">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card text-center">
              <p className="text-white/80 mb-6">
                Перед тобой карточки истории вперемешку. Нажимай на них по порядку — от начала до конца — и
                собери историю из четырёх шагов правильно!
              </p>
