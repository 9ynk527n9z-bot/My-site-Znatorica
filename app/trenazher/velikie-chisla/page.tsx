...
type QuestionType = 'thousands' | 'unitsClass';
...
const QUESTION_TEXT: Record<QuestionType, (n: number) => string> = {
  thousands: (n) => `Сколько тысяч в числе ${formatNumber(n)}?`,
  unitsClass: (n) => `Сколько единиц в классе единиц числа ${formatNumber(n)}?`,
};

function makeRound(): Round {
  const number = randInt(1000, 999999);
  const thousands = Math.floor(number / 1000);
  const unitsClass = number % 1000;

  const types: QuestionType[] = ['thousands', 'unitsClass'];
  const type = types[randInt(0, 1)];
  const answer = type === 'thousands' ? thousands : unitsClass;

  const minValue = 0;
  const maxValue = type === 'thousands' ? 999 : 999;
  ...
  return { number, type, answer, options };
}

const ROUNDS_PER_SESSION = 10;

export default function VelikieChislaTrainerPage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🔢 Большие числа</h1>
      </div>

      <TrainerGate type="trainer:velikie-chisla">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card mb-8 text-center">
              <p className="text-lg text-white/90 mb-6">
                Определи, сколько тысяч в числе или сколько единиц в классе единиц. Выбери правильный ответ из вариантов!
              </p>
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-5xl font-black text-[#3a1c6e] mb-6">{formatNumber(current.number)}</p>

              <p className="text-xl font-bold text-[#3a1c6e] mb-8">{QUESTION_TEXT[current.type](current.number)}</p>
                ...
                {current.options.map((opt, idx) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!!feedback}
                      className={`flex items-center justify-center py-6 rounded-2xl border-4 text-3xl font-black transition-all ${
                        showState
                          ? isRight
