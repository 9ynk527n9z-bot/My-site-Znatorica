...
import { ZHI_SHI_WORDS, type ZhiShiWord } from '@/lib/zhi-shi-1klass';
...
function makeSession(): ZhiShiWord[] {
  return shuffle(ZHI_SHI_WORDS).slice(0, ROUNDS_PER_SESSION);
}

export default function ZhiShiChaSchaTrainerPage() {
  const [started, setStarted] = useState(false);
  const [finished, setFinished] = useState(false);
  const [session, setSession] = useState<ZhiShiWord[]>([]);
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">📝 Жи-ши, ча-ща, чу-щу</h1>
      </div>

      <TrainerGate type="trainer:zhi-shi-cha-scha">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card text-center py-10">
              <p className="text-3xl font-black text-white mb-4">📝 Проверь себя!</p>
              <p className="text-white/70 mb-8 text-lg">
                Вставь пропущенную букву в слово. Помни: жи-ши пишем с И, ча-ща — с А, чу-щу — с У.
                Это правило нужно запомнить, оно не проверяется ударением!
              </p>
              ...
              <div className="min-h-[3rem]">
                ...
                {feedback === 'wrong' && (
                  <p className="text-red-500 font-bold text-lg">
                    Правильно: «{current.fullWord}» — {current.rule}.
                  </p>
                )}
              </div>
            </div>
