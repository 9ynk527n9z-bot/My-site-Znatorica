...
type QuestionType = 'hundreds' | 'tens' | 'units';
...
function makeRound(): Round {
  ...
  const types: QuestionType[] = ['hundreds', 'tens', 'units'];
  const type = types[randInt(0, 2)];
  const answer = type === 'hundreds' ? hundreds : type === 'tens' ? tens : units;

  const minValue = 0;
  const maxValue = type === 'hundreds' ? 9 : 9;
...
export default function TrekhznachnyeTrainerPage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-white/15 px-6 py-4">
        ...
        <h1 className="text-2xl font-bold mt-2">🔢 Трёхзначные числа</h1>
      </div>

      <TrainerGate type="trainer:trekhznachnye">
        <div className="max-w-2xl mx-auto py-8 px-6">
          {!started && (
            <div className="card mb-8 text-center">
              <p className="text-lg text-white/90 mb-6">
                Определи, сколько сотен, десятков и единиц в трёхзначном числе. Выбери правильный ответ из вариантов!
              </p>
          ...
          {started && !finished && current && (
            <div className="card bg-white text-center">
              ...
              <p className="text-6xl font-black text-[#3a1c6e] mb-6">{current.number}</p>

              <p className="text-xl font-bold text-[#3a1c6e] mb-8">
                Сколько {QUESTION_LABELS[current.type]} в числе {current.number}?
              </p>
