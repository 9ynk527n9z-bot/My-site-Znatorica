    ...
    <div className="segment-page-no-decor bg-[#28134f] min-h-screen">
      <div className="bg-gradient-to-r from-violet/20 to-orange/20 border-b border-[#2D2350] py-12 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-3xl font-bold mb-4">📚 1 класс</h1>
      ...
      <div className="max-w-6xl mx-auto py-12 px-6">
        {/* Рендерим темы ВСЕХ предметов сразу (не только активной вкладки) —
            ...
            поисковик просто не находит темы неактивных вкладок — там нет
            ссылки на них ни с одной другой проиндексированной страницы. */}
        {Object.entries(subjects).map(([key, subject]) => (
          <div key={key} className={activeSubject === key ? '' : 'hidden'}>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {subject.topics.map((topic) => (
                <Link
                  key={topic.slug}
                  href={'href' in topic && topic.href ? topic.href : `/1-klass/${key}/${topic.slug}`}
                  className="group bg-indigo-500/15 border border-[#2D2350] rounded-lg p-6 hover:border-orange hover:shadow-lg hover:shadow-orange/20 transition-all"
                >
                  ...
                  <div className="flex gap-3 text-base">
                    {!('href' in topic && topic.href) && (
                      <span className="bg-lime-400/25 text-lime-100 px-3 py-1 rounded">📝 Теория</span>
                    )}
                    <span className="bg-fuchsia-500/30 text-fuchsia-50 px-3 py-1 rounded">🎮 Тренажёр</span>
                  </div>
                </Link>
              ))}
            </div>
