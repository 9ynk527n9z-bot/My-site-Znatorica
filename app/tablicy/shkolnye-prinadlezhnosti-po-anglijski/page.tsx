...
const LEFT = ENGLISH_SCHOOL.slice(0, 10);
const RIGHT = ENGLISH_SCHOOL.slice(10);
...
const PALETTE = [
  ...
  { pale: '#fdf4ff', border: '#f0abfc' },
];

function SchoolColumn({ items, monochrome, startIndex }: { items: typeof ENGLISH_SCHOOL; monochrome: boolean; startIndex: number }) {
  return (
    <div className="flex flex-1 flex-col gap-1.5">
      {items.map((s, i) => {
        ...
        return (
          ...
          >
            <p className="w-9 shrink-0 text-center text-[28px]">{monochrome ? '' : s.emoji}</p>
            <div className="min-w-0">
              <p className="truncate text-[17px] font-bold text-slate-800">{s.word}</p>
              <p className="truncate text-[13px] font-semibold text-slate-400">{s.translation}</p>
            </div>
            <p className="ml-auto shrink-0 text-[14px] font-semibold text-slate-500">{s.transcription}</p>
          </div>
        );
      })}
    </div>
...
export default function EnglishSchoolThingsTablePage() {
    ...
    <main className="min-h-screen px-4 py-12 sm:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <header className="no-print mt-4 mb-8 max-w-3xl">
          ...
          <h1 className="text-2xl font-black leading-tight sm:text-3xl">School Things</h1>
          <p className="mt-3 text-xl text-white/70 leading-relaxed">
            20 школьных принадлежностей на английском языке с транскрипцией и переводом — в два столбика.
          </p>
        </header>
        ...
        <section className="card print-page bg-white !p-4 text-slate-900 sm:!p-6">
          <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <ExportToolbar targetRef={printRef} filename="shkolnye-prinadlezhnosti-po-anglijski" />
          </div>
          ...
          >
            <header className={`mb-5 rounded-2xl px-5 py-4 text-center ${monochrome ? 'border-2 border-slate-700' : 'bg-gradient-to-r from-violet-100 via-fuchsia-50 to-orange-50'}`}>
              ...
              <h2 className="mt-1 text-3xl font-black text-slate-900">School Things</h2>
              <p className="mt-1 text-[23px] font-semibold text-slate-500">Школьные принадлежности</p>
            </header>

            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <SchoolColumn items={LEFT} monochrome={monochrome} startIndex={0} />
              <SchoolColumn items={RIGHT} monochrome={monochrome} startIndex={LEFT.length} />
            </div>
        ...
        <section className="no-print mx-auto mt-8 max-w-3xl rounded-2xl border border-white/15 bg-white/[0.06] p-6">
          ...
          <nav className="mt-5 flex flex-wrap gap-3" aria-label="Материалы по теме">
            <Link href="/trenazher/english-school" className="btn-primary px-5 py-2.5 text-sm">Тренажёр: School Things</Link>
          </nav>
        </section>
      </div>
    </main>
