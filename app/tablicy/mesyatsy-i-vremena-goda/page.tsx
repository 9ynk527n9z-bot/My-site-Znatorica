...
const MONTHS: { month: string; season: Season; emoji: string }[] = [
...
const SEASON_STYLE: Record<Season, { pale: string; border: string; text: string }> = {
  Зима: { pale: '#eff6ff', border: '#93c5fd', text: '#1d4ed8' },
  Весна: { pale: '#f0fdf4', border: '#86efac', text: '#15803d' },
  Лето: { pale: '#fefce8', border: '#fde047', text: '#a16207' },
  Осень: { pale: '#fff7ed', border: '#fdba74', text: '#c2410c' },
};

export default function MonthsSeasonsTablePage() {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="no-print mt-4 mb-8 max-w-3xl">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold text-violet-100">
            🌍 Окружающий мир · 5–8 лет
          </div>
          <h1 className="text-3xl font-black leading-tight sm:text-4xl">Месяцы и времена года</h1>
          <p className="mt-3 text-lg text-white/70 leading-relaxed">
            12 месяцев, сгруппированных по временам года, с наглядными значками.
          </p>
        </header>
        ...
        <section className="card print-page bg-white !p-4 text-slate-900 sm:!p-6">
          <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <ExportToolbar targetRef={printRef} filename="mesyatsy-i-vremena-goda" />
          </div>
          ...
          >
            <header className={`mb-5 rounded-2xl px-5 py-4 text-center ${monochrome ? 'border-2 border-slate-700' : 'bg-gradient-to-r from-violet-100 via-fuchsia-50 to-orange-50'}`}>
              ...
              <h2 className="mt-1 text-3xl font-black text-slate-900">Месяцы и времена года</h2>
            </header>

            <div
              className="mb-3 grid grid-cols-4 gap-2 rounded-xl px-3 py-2"
              style={{ backgroundColor: monochrome ? '#334155' : '#4C1D95' }}
            >
              <p className="col-span-4 text-center text-base font-black uppercase tracking-wide text-white sm:col-span-1">Время года</p>
              <p className="col-span-4 text-center text-base font-black uppercase tracking-wide text-white sm:col-span-3">Месяцы</p>
            </div>

            {(['Зима', 'Весна', 'Лето', 'Осень'] as Season[]).map((season, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <p className="text-4xl">{monochrome ? '' : season === 'Зима' ? '⛷️' : season === 'Весна' ? '🌼' : season === 'Лето' ? '🏊' : '🍄'}</p>
                      <p className="text-xl font-black text-white">{season}</p>
                    </div>
                    {months.map((m) => (
                      <div key={m.month} className="rounded-xl bg-white/70 px-2 py-2 text-center">
                        <p className="text-3xl">{monochrome ? '' : m.emoji}</p>
                        <p className="mt-0.5 text-lg font-bold text-slate-700">{m.month}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section className="no-print mx-auto mt-8 max-w-3xl rounded-2xl border border-white/15 bg-white/[0.06] p-6">
          ...
          <p className="mt-3 leading-relaxed text-white/70">
            Повторяй месяцы по порядку, называя время года для каждого. Обрати внимание, что декабрь относится
            к зиме, хотя год ещё не закончился — год и зима начинаются не одновременно.
          </p>
          <nav className="mt-5 flex flex-wrap gap-3" aria-label="Материалы по месяцам и временам года">
            <Link href="/trenazher/mesyatsy-i-sezony" className="btn-primary px-5 py-2.5 text-sm">
              Тренажёр «Месяцы и времена года»
            </Link>
          </nav>
        </section>
      </div>
