...
const DAYS = [
  { num: 1, name: 'Понедельник', weekend: false },
  { num: 2, name: 'Вторник', weekend: false },
  { num: 3, name: 'Среда', weekend: false },
  { num: 4, name: 'Четверг', weekend: false },
  { num: 5, name: 'Пятница', weekend: false },
  { num: 6, name: 'Суббота', weekend: true },
  { num: 7, name: 'Воскресенье', weekend: true },
];

export default function WeekdaysTablePage() {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="no-print mt-4 mb-8 max-w-3xl">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold text-violet-100">
            🌍 Окружающий мир · 4–7 лет
          </div>
          <h1 className="text-2xl font-black leading-tight sm:text-3xl">Дни недели</h1>
          <p className="mt-3 text-lg text-white/70 leading-relaxed">
            7 дней недели по порядку: будни и выходные, с наглядными значками.
          </p>
        </header>
        ...
        <section className="card print-page bg-white !p-4 text-slate-900 sm:!p-6">
          <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <ExportToolbar targetRef={printRef} filename="dni-nedeli" />
          </div>
          ...
          >
            <header className={`mb-5 rounded-2xl px-5 py-4 text-center ${monochrome ? 'border-2 border-slate-700' : 'bg-gradient-to-r from-violet-100 via-fuchsia-50 to-orange-50'}`}>
              ...
              <h2 className="mt-1 text-3xl font-black text-slate-900">Дни недели</h2>
            </header>
            ...
            <div className="flex flex-col gap-2.5">
              {DAYS.map((d) => (
                ...
                >
                  <p className="w-8 shrink-0 text-3xl font-black" style={{ color: monochrome ? '#0f172a' : d.weekend ? '#c2410c' : '#1d4ed8' }}>{d.num}</p>
                  <p className="flex-1 text-xl font-bold text-slate-800">{d.name}</p>
                  {d.weekend && <p className="shrink-0 text-base font-semibold text-slate-500">выходной</p>}
                </div>
              ))}
            </div>
        ...
        <section className="no-print mx-auto mt-8 max-w-3xl rounded-2xl border border-white/15 bg-white/[0.06] p-6">
          ...
          <p className="mt-3 leading-relaxed text-white/70">
            Повторяй дни строго по порядку — так легче запомнить последовательность. Обрати внимание,
            что суббота и воскресенье — выходные, а остальные пять дней — будние.
          </p>
        </section>
      </div>
    </main>
