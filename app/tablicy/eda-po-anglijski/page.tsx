...
const LEFT = ENGLISH_FOOD.slice(0, 10);
const RIGHT = ENGLISH_FOOD.slice(10);
...
function FoodColumn({ items, monochrome, startIndex }: { items: typeof ENGLISH_FOOD; monochrome: boolean; startIndex: number }) {
  return (
    <div className="flex flex-1 flex-col gap-1.5">
      {items.map((f, i) => {
        ...
        return (
          ...
          >
            <p className="w-9 shrink-0 text-center text-[28px]">{monochrome ? '' : f.emoji}</p>
            <div className="min-w-0">
              <p className="truncate text-[17px] font-bold text-slate-800">{f.word}</p>
              <p className="truncate text-[13px] font-semibold text-slate-400">{f.translation}</p>
            </div>
            <p className="ml-auto shrink-0 text-[14px] font-semibold text-slate-500">{f.transcription}</p>
          </div>
        );
      })}
    </div>
...
export default function EnglishFoodTablePage() {
    ...
    <main className="min-h-screen px-4 py-12 sm:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <header className="no-print mt-4 mb-8 max-w-3xl">
          ...
          <h1 className="text-2xl font-black leading-tight sm:text-3xl">Food</h1>
          <p className="mt-3 text-xl text-white/70 leading-relaxed">
            20 продуктов и блюд на английском языке с транскрипцией и переводом — в два столбика.
          </p>
        </header>
        ...
        <section className="card print-page bg-white !p-4 text-slate-900 sm:!p-6">
          <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <ExportToolbar targetRef={printRef} filename="eda-po-anglijski" />
          </div>
          ...
          >
            <header className={`mb-5 rounded-2xl px-5 py-4 text-center ${monochrome ? 'border-2 border-slate-700' : 'bg-gradient-to-r from-violet-100 via-fuchsia-50 to-orange-50'}`}>
              ...
              <h2 className="mt-1 text-3xl font-black text-slate-900">Food</h2>
              <p className="mt-1 text-[23px] font-semibold text-slate-500">Еда на английском</p>
            </header>

            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <FoodColumn items={LEFT} monochrome={monochrome} startIndex={0} />
              <FoodColumn items={RIGHT} monochrome={monochrome} startIndex={LEFT.length} />
            </div>
        ...
        <section className="no-print mx-auto mt-8 max-w-3xl rounded-2xl border border-white/15 bg-white/[0.06] p-6">
          ...
          <nav className="mt-5 flex flex-wrap gap-3" aria-label="Материалы по теме">
            <Link href="/trenazher/english-food" className="btn-primary px-5 py-2.5 text-sm">Тренажёр: Food</Link>
          </nav>
        </section>
      </div>
    </main>
