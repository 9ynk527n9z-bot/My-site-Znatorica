...
export default function TopicQuiz({ topic }: { topic: string }) {
  ...
  if (!round) {
    return (
      <div className="bg-gradient-to-r from-orange/20 to-violet/20 border border-orange rounded-lg p-8 text-center">
        ...
        <button onClick={start} className="btn-primary text-lg px-8 py-4">
          🎮 Начать викторину
        </button>
      </div>
    );
          ...
          {score === round.length
            ...
            : score >= 7
              ...
              : score >= 5
                ...
                : 'Не расстраивайся — попробуй ещё раз, получится лучше!'}
        </p>
        <button onClick={start} className="btn-primary text-lg px-8 py-4">
          🔄 Играть ещё раз
        </button>
  ...
  return (
    <div className="bg-[#2A1B4D] border border-[#2D2350] rounded-lg p-6 sm:p-8">
      ...
      <p className="text-xl sm:text-2xl font-bold mb-6 leading-relaxed break-words">{q.prompt}</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {q.options.map((opt, i) => {
          let cls = 'bg-black border border-[#2D2350] hover:border-orange text-white';
          if (answered) {
            if (i === q.correct) cls = 'bg-green-600 border border-green-500 text-white';
            else if (i === picked) cls = 'bg-red-600/70 border border-red-500 text-white';
            else cls = 'bg-black border border-[#2D2350] text-gray-500';
          }
          return (
            <button
              key={i}
              onClick={() => choose(i)}
              disabled={answered}
              className={`px-4 py-4 rounded-lg font-bold text-lg transition-colors ${cls}`}
            >
              {opt}
            </button>
          );
      ...
      {answered && (
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className={`font-bold ${picked === q.correct ? 'text-green-400' : 'text-red-400'}`}>
